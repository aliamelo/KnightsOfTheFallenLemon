...
public class LayoutAnchor : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (parentRT == null)
            Debug.LogError("Ce composant a besoin d'un parent RectTransform pour pouvoir fonctionner.", gameObject);
    }

    //Pour avoir la position du texte
    Vector2 GetPosition (RectTransform rt, TextAnchor anchor)
    {
        Vector2 retValue = Vector2.zero;

        //Deux switch différents => un par axe si jamais les 2 doivent être modifiés en même temps
        switch(anchor)
        {
    ...
    //Pour (vraiment) positionner le texte
    ...
    //Pour animer le mouvement de RectTransform
