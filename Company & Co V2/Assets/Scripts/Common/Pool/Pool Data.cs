...
public class GameObjectPoolController : MonoBehaviour
{
    ...
    {
        if (!pools.ContainsKey(key)) //Evite le return null
            return;
        PoolData data = pools[key];
        data.maxCount = maxCount;
    }

    //Ajoute key/Gameobject au dico si existe pas déjà;
    public static bool AddEntry(string key, GameObject prefab, int prepopulate, int maxCount)
    {
        ...
        data.prefab = prefab;
        data.maxCount = maxCount;
        data.pool = new Queue<Poolable>(prepopulate); //prepopulate : capacité initiale de la queue
        pools.Add(key, data);
